...
public class ArrayIntSolutionType extends SolutionType {
	...
	public Variable[] createVariables(int size) {
		Variable [] variables = new Variable[size];
        for (int i=0;i<size;i++)
		variables[i] = new ArrayInt(problem_.getNumberOfVariables(), problem_);
		return variables ;
	} // createVariables
	...
	public Variable[] copyVariables(Variable[] vars) {
		...
		for (int i=0;i<vars.length;i++) variables[i] = vars[i].deepCopy();
